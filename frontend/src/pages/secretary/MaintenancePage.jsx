...
import {
    ...
    Tooltip, Skeleton, Dialog, DialogTitle,
...
const MaintenancePage = () => {
    ...
    const fetchFlatInfo = async (wingName, flatNumber) => {
        ...
        try {
            ...
            if (!res.data) {
                setFlatInfoError(`Flat ${wingName}-${flatNumber} is not registered in the system.`);
            } else if (!res.data.residentId) {
                setFlatInfoError(`Flat ${wingName}-${flatNumber} has no resident assigned.`);
            } else {
    ...
    const handleGenerateBill = async () => {
        ...
        try {
            ...
            showSuccess(`Bill generated for ${flatInfo.residentName} — ${flatInfo.flatNumber}`);
        ...
        bills.length === 0 ? (
            ...
            <TableContainer sx={{ overflowX: 'auto' }}>
                <Table size="small">
                    ...
                    <TableBody>
                        {bills.map((bill) => (
                            <TableRow key={bill.id} hover sx={{ '&:hover': { bgcolor: '#f8fbff' } }}>
                                <TableCell sx={cellSx}>
                                    <Chip label={bill.flatNumber || '—'} size="small" sx={{ bgcolor: '#e0f2fe', color: '#0891b2', fontWeight: 700, fontSize: '0.7rem', fontFamily: 'Inter, sans-serif', height: 22 }} />
                                </TableCell>
                                <TableCell sx={cellSx}>{bill.residentName || '—'}</TableCell>
                                <TableCell sx={cellSx}>
                                    {MONTHS[(bill.billMonth || 1) - 1]} {bill.billYear}
                                </TableCell>
                                <TableCell sx={cellSx}>₹{bill.baseAmount?.toLocaleString('en-IN')}</TableCell>
                                <TableCell sx={cellSx}>
                                    <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.82rem', color: bill.fineAmount > 0 ? '#dc2626' : '#1e293b', fontWeight: bill.fineAmount > 0 ? 700 : 400 }}>
                                        ₹{bill.fineAmount?.toLocaleString('en-IN')}
                                    </Typography>
                                </TableCell>
                                <TableCell sx={cellSx}>
                                    <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.82rem', fontWeight: 700, color: '#1e293b' }}>
                                ...
                                <TableCell sx={cellSx}><StatusChip status={bill.status} /></TableCell>
                                <TableCell align="center" sx={{ py: 1 }}>
                                    <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'center' }}>
                                        <Tooltip title="View Details">
                                            <IconButton size="small" onClick={() => { setSelectedBill(bill); setDetailOpen(true); }}
                                        ...
                                        {bill.status === 'PENDING' && (
                                            <Tooltip title="Mark as Paid">
                                                <IconButton size="small" onClick={() => handleMarkPaid(bill.id)} disabled={actionLoading}
    ...
    return (
        <Box>
            ...
            <Box sx={{ mb: 2.5, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                ...
                <Button variant="outlined" size="small" startIcon={<SettingsIcon fontSize="small" />}
                    onClick={() => setSettingsOpen(true)}
                    sx={{ borderRadius: 2, borderColor: '#0891b2', color: '#0891b2', fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: '0.8rem', px: 2, mr: 1 }}>
                    Settings
                </Button>
                <Button variant="contained" size="small" startIcon={<AddIcon fontSize="small" />}
                    onClick={() => setGenerateOpen(true)}
                    sx={{ bgcolor: '#0891b2', borderRadius: 2, fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: '0.8rem', px: 2, boxShadow: '0 2px 6px rgba(8,145,178,0.25)', '&:hover': { bgcolor: '#0e7490' } }}>
                    Generate Bill
                </Button>
            </Box>
            ...
            {settings && (
                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2, mb: 2.5 }}>
                    {[
                        { label: 'Monthly Amount', value: `₹${Number(settings.maintenanceAmount).toLocaleString('en-IN')}`, color: '#0891b2', bg: '#e0f2fe' },
                        { label: 'Fine Per Day', value: `₹${Number(settings.dueFinePerDay).toLocaleString('en-IN')}`, color: '#dc2626', bg: '#fee2e2' },
                        { label: 'Pending Bills', value: pendingBills.length, color: '#d97706', bg: '#fef3c7' },
                        { label: 'Paid Bills', value: paidBills.length, color: '#059669', bg: '#dcfce7' },
                    ].map((stat) => (
                        <Paper key={stat.label} elevation={0} sx={{ p: 2, borderRadius: 3, border: '1px solid #e0f2fe', boxShadow: '0 2px 12px rgba(8,145,178,0.06)' }}>
                            <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.7rem', fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', mb: 0.5 }}>
                                {stat.label}
                            </Typography>
                            <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '1.4rem', fontWeight: 800, color: stat.color }}>
                                {loading ? '—' : stat.value}
                            </Typography>
                        </Paper>
                    ))}
            ...
            <Paper elevation={0} sx={{ borderRadius: 3, border: '1px solid #e0f2fe', overflow: 'hidden', boxShadow: '0 2px 12px rgba(8,145,178,0.06)' }}>
                ...
                {loading ? <LoadingSkeleton /> : (
                    tab === 0 ? <BillsTable bills={allBills} /> :
                        tab === 1 ? <BillsTable bills={pendingBills} /> :
            ...
            <Dialog open={detailOpen} onClose={() => setDetailOpen(false)} maxWidth="xs" fullWidth
                ...
                {selectedBill && (
                    <DialogContent sx={{ pt: 2 }}>
                        {[
                            ...
                            ['Bill Month', `${MONTHS[(selectedBill.billMonth || 1) - 1]} ${selectedBill.billYear}`],
                            ['Due Date', selectedBill.dueDate ? new Date(selectedBill.dueDate).toLocaleDateString('en-IN') : '—'],
                            ['Base Amount', `₹${selectedBill.baseAmount?.toLocaleString('en-IN')}`],
                            ['Fine Amount', `₹${selectedBill.fineAmount?.toLocaleString('en-IN')}`],
                            ['Total Amount', `₹${selectedBill.totalAmount?.toLocaleString('en-IN')}`],
                            ['Status', selectedBill.status],
                            ...(selectedBill.paidAt ? [['Paid On', new Date(selectedBill.paidAt).toLocaleString('en-IN')]] : []),
                            ['Generated On', new Date(selectedBill.createdAt).toLocaleString('en-IN')],
                        ].map(([label, value]) => (
                            <Box key={label} sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid #f1f5f9', alignItems: 'center' }}>
                                <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', color: '#64748b', fontWeight: 500 }}>
                                    {label}
                                </Typography>
                                <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', color: label === 'Total Amount' ? '#0891b2' : '#1e293b', fontWeight: label === 'Total Amount' ? 800 : 600 }}>
                                    {value}
                                </Typography>
                            </Box>
                        ))}
                ...
                <DialogActions sx={{ p: 2, gap: 1, borderTop: '1px solid #e0f2fe' }}>
                    ...
                    {selectedBill?.status === 'PENDING' && (
                        <Button variant="contained" onClick={() => handleMarkPaid(selectedBill?.id)} disabled={actionLoading}
            ...
            <Dialog open={settingsOpen} onClose={() => setSettingsOpen(false)} maxWidth="xs" fullWidth
                ...
                <DialogContent sx={{ pt: 2.5 }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        <TextField label="Monthly Amount (₹) *" value={settingsForm.maintenanceAmount}
                            onChange={(e) => { if (/^\d*\.?\d*$/.test(e.target.value)) setSettingsForm({ ...settingsForm, maintenanceAmount: e.target.value }); }}
                            size="small" fullWidth
                            error={!!settingsErrors.maintenanceAmount} helperText={settingsErrors.maintenanceAmount}
                            sx={fieldStyle} />
                        <TextField label="Fine Per Day (₹) *" value={settingsForm.dueFinePerDay}
                            onChange={(e) => { if (/^\d*\.?\d*$/.test(e.target.value)) setSettingsForm({ ...settingsForm, dueFinePerDay: e.target.value }); }}
                            size="small" fullWidth
                            error={!!settingsErrors.dueFinePerDay} helperText={settingsErrors.dueFinePerDay}
                            sx={fieldStyle} />
                        <TextField label="Payment Validity Days *" value={settingsForm.validityDays}
                            onChange={(e) => { if (/^\d*$/.test(e.target.value)) setSettingsForm({ ...settingsForm, validityDays: e.target.value }); }}
                            size="small" fullWidth
                            error={!!settingsErrors.validityDays}
                            helperText={settingsErrors.validityDays || 'Bill due on this day of each month'}
                            sx={fieldStyle} />
                    </Box>
                </DialogContent>
                <DialogActions sx={{ p: 2, gap: 1, borderTop: '1px solid #e0f2fe' }}>
                    ...
                    <Button variant="contained" onClick={handleUpdateSettings} disabled={actionLoading}
            ...
            {/* Generate Bill Modal */}
            <Dialog open={generateOpen} onClose={() => {
                setGenerateOpen(false);
                setGenerateForm({ wingName: '', flatNumber: '', billMonth: new Date().getMonth() + 1, billYear: new Date().getFullYear() });
                setGenerateErrors({});
                setFlatInfo(null);
                setFlatInfoError('');
            }}
                ...
                <DialogContent sx={{ pt: 2.5 }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        <Box sx={{ p: 1.5, bgcolor: '#f0f9ff', borderRadius: 2, border: '1px solid #e0f2fe' }}>
                            <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.75rem', color: '#64748b', mb: 0.3 }}>
                                Current Settings
                            </Typography>
                            <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.82rem', fontWeight: 600, color: '#0891b2' }}>
                                ₹{settings?.maintenanceAmount?.toLocaleString('en-IN')} / month • Fine ₹{settings?.dueFinePerDay}/day after {settings?.validityDays}th
                            </Typography>
                        </Box>
                        ...
                        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
                            <FormControl size="small" sx={fieldStyle}>
                                ...
                                <Select
                                    ...
                                    onChange={(e) => {
                                        setGenerateForm({ ...generateForm, wingName: e.target.value });
                                        setFlatInfo(null);
                                        setFlatInfoError('');
                                    ...
                                    {['A', 'B', 'C', 'D', 'E'].map(w => (
                                        <MenuItem key={w} value={w} sx={{ fontFamily: 'Inter, sans-serif' }}>Wing {w}</MenuItem>
                                    ))}
                                </Select>
                            </FormControl>
                            ...
                            <TextField
                                ...
                                onChange={(e) => {
                                    if (!/^\d*$/.test(e.target.value) || e.target.value.length > 4) return;
                                    setGenerateForm({ ...generateForm, flatNumber: e.target.value });
                                    setFlatInfo(null);
                                    setFlatInfoError('');
                                }}
                                onBlur={() => {
                                    if (generateForm.wingName && generateForm.flatNumber) {
                                        fetchFlatInfo(generateForm.wingName, generateForm.flatNumber);
                                    }
                                }}
                                size="small"
                        ...
                        {/* Flat loading */}
                        {flatInfoLoading && (
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.5, py: 0.8, bgcolor: '#f8fbff', borderRadius: 1.5, border: '1px solid #e0f2fe' }}>
                        ...
                        {/* Flat error */}
                        {flatInfoError && (
                            <Box sx={{ px: 1.5, py: 0.8, bgcolor: '#fef2f2', borderRadius: 1.5, border: '1px solid #fecaca' }}>
                                <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.75rem', color: '#991b1b', fontWeight: 600 }}>
                                    {flatInfoError}
                                </Typography>
                            </Box>
                        )}
                        ...
                        {flatInfo && (
                            <Box sx={{ p: 1.5, bgcolor: '#f0fdf4', borderRadius: 2, border: '1px solid #bbf7d0' }}>
                                <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.7rem', fontWeight: 700, color: '#059669', textTransform: 'uppercase', letterSpacing: '0.05em', mb: 1 }}>
                                    ✓ Flat Found — Bill will be generated for:
                                </Typography>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                    <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', color: '#64748b' }}>Flat</Typography>
                                    <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', fontWeight: 700, color: '#1e293b' }}>{flatInfo.flatNumber}</Typography>
                                </Box>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                    <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', color: '#64748b' }}>Resident</Typography>
                                    <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', fontWeight: 700, color: '#1e293b' }}>{flatInfo.residentName}</Typography>
                                </Box>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <Typography sx={{ fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', color: '#64748b' }}>Type</Typography>
                                    <Chip label={flatInfo.residentType} size="small" sx={{ bgcolor: flatInfo.residentType === 'TENANT' ? '#f3e8ff' : '#e0f2fe', color: flatInfo.residentType === 'TENANT' ? '#7c3aed' : '#0891b2', fontWeight: 700, fontSize: '0.68rem', fontFamily: 'Inter, sans-serif', height: 20 }} />
                                </Box>
                            </Box>
                        )}
                        ...
                        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
                            <TextField
                                ...
                                onChange={(e) => { const v = Number(e.target.value); if (v >= 1 && v <= 12) setGenerateForm({ ...generateForm, billMonth: v }); }}
                            ...
                            <TextField
                                ...
                                onChange={(e) => { if (/^\d*$/.test(e.target.value)) setGenerateForm({ ...generateForm, billYear: e.target.value }); }}
                                size="small"
                                sx={fieldStyle}
                            />
                        </Box>
                ...
                <DialogActions sx={{ p: 2, gap: 1, borderTop: '1px solid #e0f2fe' }}>
                    <Button onClick={() => { setGenerateOpen(false); setGenerateForm({ flatId: '', residentId: '', billMonth: new Date().getMonth() + 1, billYear: new Date().getFullYear() }); setGenerateErrors({}); }}
                        sx={{ fontFamily: 'Inter, sans-serif', color: '#64748b', textTransform: 'none' }}>Cancel</Button>
                    <Button variant="contained" onClick={handleGenerateBill} disabled={actionLoading}
                        sx={{ fontFamily: 'Inter, sans-serif', textTransform: 'none', bgcolor: '#0891b2', borderRadius: 2, '&:hover': { bgcolor: '#0e7490' } }}>
